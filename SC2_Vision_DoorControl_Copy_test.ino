...
const int Driver_open_target = 4060; //Variable to define set target for driver door open
const int Driver_close_target = 25 ; //Variable to define set target for driver door close

//----LUCY YANG----
const int Passenger_open_target = 4060; //Variable to define set target for passenger open
const int Passenger_close_target = 25; //Variable to define set target for passenger close
//-----------------
...
void passenger_door_switch(){ //Command when passenger door switch is pressed
  if (passenger_moving)
  {
   ...
   else if (passenger_feedback < 3900)
   {
    Serial.println("opening");
    autoopen_passenger();
    passenger_moving = true;
...
void driver_moving_check(){ //Constant flag check for driver switch, will enable to use physical switch as stop button
   uint16_t driver_feedback = jrk1.getScaledFeedback(); 
   if (driver_feedback > 3950)
   {
    driver_moving = false;
   }
   else if (driver_feedback < 40)
   {
    driver_moving = false;
   }
}
...
void passenger_moving_check(){ //Constant flag check for driver switch, will enable to use physical switch as stop button
   uint16_t passenger_feedback = jrk2.getScaledFeedback(); 
   if (passenger_feedback > 3950)
   {
    passenger_moving = false;
   }
   else if (passenger_feedback < 40)
   {
    passenger_moving = false;
   }
}
