...
const int Driver_open_target = 3080; //Variable to define set target for driver door open
const int Driver_close_target = 0 ; //Variable to define set target for driver door close
const int Passenger_open_target = 2930; //Variable to define set target for passenger open
const int Passenger_close_target = 112; //Variable to define set target for passenger close
...
void passenger_door_switch(){ //Command when passenger door switch is pressed
  if (passenger_moving)
  {
   ...
   else if (passenger_feedback < 2850)
   {
    Serial.println("opening");
    autoopen_passenger();
    passenger_moving = true;
...
void passenger_moving_check(){ //Constant flag check for driver switch, will enable to use physical switch as stop button
   ...
   else if (passenger_feedback < 150)
   {
    passenger_moving = false;
   }
}
...
void door_set_speed_forward()
{
  jrk1.setMaxAccelerationForward(400);
  jrk1.setMaxDecelerationForward(400);
  jrk2.setMaxAccelerationForward(400);
  jrk2.setMaxDecelerationForward(400);
}

void door_set_speed_reverse()
{
  jrk1.setMaxAccelerationReverse(200);
  jrk1.setMaxDecelerationReverse(200);
  jrk1.setMaxDutyCycleReverse(500);
  jrk2.setMaxAccelerationReverse(200);
  jrk2.setMaxDecelerationReverse(200);
  jrk2.setMaxDutyCycleReverse(500);
}
